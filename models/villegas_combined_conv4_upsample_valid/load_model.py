...
class Network(NNBase):
    ...
    def f_dec(self, input):
        ...
        with tf.variable_scope('f_dec'):
            deconv4_4 = deconv(tf.image.resize_images(input,[26,26],tf.image.ResizeMethod.NEAREST_NEIGHBOR), 3, int(512*width), 1, 'deconv4_4', padding='VALID')            
            deconv4_3 = deconv(deconv4_4, 3, int(512*width), 1, 'deconv4_3')
            deconv4_2 = deconv(deconv4_3, 3, int(512*width), 1, 'deconv4_2')
            deconv4_1 = deconv(tf.image.resize_images(deconv4_2,[54,54],tf.image.ResizeMethod.NEAREST_NEIGHBOR), 3, int(256*width), 1, 'deconv4_1', padding='VALID')
                      
            deconv3_3 = deconv(deconv4_1, 3, int(256*width), 1, 'deconv3_3')
            deconv3_2 = deconv(deconv3_3, 3, int(256*width), 1, 'deconv3_2')
            deconv3_1 = deconv(tf.image.resize_images(deconv3_2,[110,110],tf.image.ResizeMethod.NEAREST_NEIGHBOR), 3, int(128*width), 1, 'deconv3_1', padding='VALID')
            
            deconv2_2 = deconv(deconv3_1, 3, int(128*width), 1, 'deconv2_2')
            deconv2_1 = deconv(tf.image.resize_images(deconv2_2,[222,222],tf.image.ResizeMethod.NEAREST_NEIGHBOR), 3, int(64*width), 1, 'deconv2_1', padding='VALID')
