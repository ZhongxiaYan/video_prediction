...
class Network(NNBase):
    def __init__(self, config, sess, dataset):
        ...
        # generator output
        f_t_n_pred_norm = self.generator_network(f_t_norm, p_t_norm, f_t_n_norm, p_t_n_norm)
        ...
        with tf.variable_scope('generator'):
            ...
            latent_loss_gen = l2_loss(self.layer_outputs['f_t_n_latent_true'], self.layer_outputs['f_t_n_latent_pred'])
            adv_loss_gen = -tf.reduce_mean(tf.log(pr_real_gen))
            
            self.loss_gen = config.l2_coef * l2_loss_gen + config.latent_coef * latent_loss_gen + config.adv_coef * adv_loss_gen
            summ_list = [
                tf.summary.scalar('l2_loss', l2_loss_gen),
                tf.summary.scalar('latent_loss', latent_loss_gen),
    ...
    def generator_network(self, f_t, p_t, f_t_n, p_t_n):
        if self.config.fix_weights_conv3:
            f_t = self.f_t
            f_t_n = self.f_t_n
        with tf.variable_scope('generator'):
    ...
    def discriminator_network(self, f, p, reuse=False):
        with tf.variable_scope('discriminator', reuse=reuse):
            ...
            conv6 = self.vgg_conv(concat, fix_weights=False)
    ...
    def f_pose(self, input, reuse=False):
        with tf.variable_scope('f_pose', reuse=reuse):
            return self.vgg_conv(input, fix_weights=False)
        
    def f_img(self, input, reuse=False):
        with tf.variable_scope('f_img', reuse=reuse):
            return self.vgg_conv(input, fix_weights=self.config.fix_weights_conv3)
        
    def vgg_conv(self, input, fix_weights=False):
        if fix_weights:
        ...
        def conv_relu(x, num_filters, name, relu=True):
        ...
        for i, depth in enumerate(depths):
            layer_num = i + 1
            if fix_weights and layer_num <= 3:
                conv_ = conv_relu
            else:
                conv_ = conv_lrelu
